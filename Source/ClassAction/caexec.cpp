...
/* Quote/QuoteStr defined in ClassAction.cpp (shared shell quoting). */
extern char Quote;
...
BOOL GetVersion( char *file,char *version ){
  ...
  if( Handle=Open(file,MODE_OLDFILE ) ){        // try to open file
    ...
    if( buffer ){
      ...
      while( (result == FALSE) && (bytes == Main->BufferSize) ){
        ...
        if( bytes>0 ){
          ...
          }while( ptr && (memcmp(ptr,"$VER",4) != 0) );      // search for VER
          if( ptr ){      
            if( (ptr=strchr(ptr,' ')) &&
...
BOOL CopyArchiveFile( char *src, char *dest ){
  ...
  if( Main->ConfirmReplace ){
    ...
    if( lock ){
      ...
      if( (Main->Transfer.RequesterResult == REPLACE_NOT) ||
          (Main->Transfer.RequesterResult == REPLACE_ONE) ){    // we must ask the user
        Main->Transfer.RequesterResult = AskReplace( src,dest,Main->Transfer.ReplaceText );
      }
      if( (Main->Transfer.RequesterResult == REPLACE_NOT) ||
          (Main->Transfer.RequesterResult == REPLACE_NONE) ) result=1;
    }
  }
...
void TryDeprotectFile( char *file ){
  ...
  if( lock ){
    ...
    if( (fib->fib_Protection & FIBF_WRITE) ||
      ...
      if( (Main->Transfer.DeprotectResult == DEPROTECT_ONE) ||
          (Main->Transfer.DeprotectResult == DEPROTECT_NOT) ){
      ...
      if( (Main->Transfer.DeprotectResult == DEPROTECT_ONE) ||
          (Main->Transfer.DeprotectResult == DEPROTECT_ALL) ){
...
int MRECopy( char *src,char *dest,Object *gauge,UBYTE mode ){
  ...
  if( info->fib_DirEntryType>0 ){
    ...
    if( result == 0 ){
      while( (childresult > -1) && ExNext(lock,info) ){
        strcpy( ptrs,info->fib_FileName );
        strcpy( ptrd,info->fib_FileName );
        childresult = MRECopy( nextsrc,nextdest,gauge,mode );
      }
      UnLock( lock );
      if( (childresult == 0) && ((mode == FT_DELETE)||(mode == FT_MOVE)||(mode == FT_MOVEAS)) ){
        childresult = DeleteFile( src );
        if( ! childresult ){
          TryDeprotectFile( src );
          if( (Main->Transfer.DeprotectResult == DEPROTECT_ONE) ||
              (Main->Transfer.DeprotectResult == DEPROTECT_ALL) ) childresult = DeleteFile( src );
        }
        if( ! childresult  ){
          char *msg=new char[512];
      ...
      }else if( childresult == -1 ) result = -1; // abort == valid for all !!
    }else UnLock( lock );
    delete [] nextsrc;
    delete [] nextdest;
  }else{
    ...
    if( mode == FT_DELETE ){
      ...
      if( !srcfile ) result = -6;
        ...
        if( result == 0 ){
          ...
          if( !dstfile ){
            ...
            if( (Main->Transfer.DeprotectResult == DEPROTECT_ONE) ||
                (Main->Transfer.DeprotectResult == DEPROTECT_ALL) ) dstfile = Open( dest,MODE_NEWFILE );
          }
          if( !dstfile ){
            char *msg=new char[512];
            result = -4;
      ...
      if( result == 0 ){
        if( copiedbytes == -1 ){
          ...
          if( (mode == FT_MOVE) || (mode == FT_MOVEAS) ) DeleteFile( src );
        }
      }
    }
  }
...
void AppendFilesToString( char *String,TFileList *list,char Specifier ){
  ...
  while( File->ln_Succ ){
    if( File->Selected ){
      if( ! isupper(Specifier) ) strcat( String,sep );
  ...
  if( String[0] && (String[strlen(String)-1] == ' ') ) String[strlen(String)-1] = 0;
}
//<
...
int ParseArgs(char *exec,char *file,TFileList *list,char *res){
  ...
  found=strstr(tmpe,"[f]"); // filename with whole path && quotes
  if(found!=NULL){
   *found=0;
   strcpy(res,tmpe);
   strcat(res,sep);
  ...
  found=strstr(tmpe,"[x]"); // filename without path && suffix with quotes
  if(found!=NULL){
    x=strchr(tmpf,'.');
    if(x!=NULL){
      suf=x+1;
...
int ParseReq(char *exec,char *fname,char **reqbuffer,char *res){
 ...
 if( (found[1] != 'O') || (reqbuffer[mode][0] == 0) ){
   if(mode<3){
     ...
     if( !reqbuffer[mode][0] ) strcpy( reqbuffer[mode],GetSrc() ); // || the source, if dest=volumes
     if( !reqbuffer[mode][0] ) strcpy( reqbuffer[mode],"Sys:" ); // || the source, if dest=volumes
     if( mode == 0 ){
       // && for a reqbuffer[i] request add the reqbuffer[i]name:
       if( ptr=FilePart(fname) ) AddPart( reqbuffer[mode],ptr,255 );
     }else if( mode == 1 ){
       if( reqbuffer[mode][strlen(reqbuffer[mode])-1] != ':' ) strcat( reqbuffer[mode],"/" );
     }
...
void InternalCommand( char *CommandLine,TFileList *list ){
 ...
 if( Count == 0 ) IntArg=Main->CurrentList;
 else{
  ...
  switch( dummy ){
    ...
    default:{
      IntArg=atoi(Arg1);
      if( IntArg == 1 ) IntArg = 1-Main->CurrentList; // destination
...
//  NotifyRequest NotifySrc,NotifyDst;
  ...
  switch( act->CD ){
    case 0:{
      if( (! lha) && FiList->Source[0] )
        strcpy( sCurrentDir,FiList->Source );
      break;
    }
    case 1:{
      if( (! lhad) && FiList->Destination[0] )
...
//  }
  ...
  do{
   ...
   while( (oktodo=ParseArgs(command,SystemCommand,FiList,result)) > -1 ){
    ...
    if( oktodo == 1 ) bFilenameUsed = TRUE;
   }
...
/*
      if( act->CdDest && (! lhad) && FiList->Destination[0] && (FiList->Destination[1] != Quote) ){
        sprintf(charbuffer,"cd %c%s%c",'"',FiList->Destination,'"');        // change dir
      }else if( (! act->CdDest) && (! lha) && FiList->Source[0] && (FiList->Source[1] != Quote) ){
...
//        free( charbuffer );
      }else if( act->Type == RUN_AREXX ) sprintf( SystemCommand,"sys:rexxc/rx %s",result );
     }

     if( (sCurrentDir[0] == 0) || !(CurrentDirLock=Lock(sCurrentDir,ACCESS_READ)) )
       CurrentDirLock=Lock("sys:",ACCESS_READ);
     if( CurrentDirLock ) OldDirLock = CurrentDir(CurrentDirLock);
     if( act->Async ){
       // do ! reread directory, so start asynchronous:
       if( SystemTags(SystemCommand,SYS_Input,handler,SYS_Output,NULL,SYS_Asynch,TRUE,
           NP_StackSize,stack,TAG_DONE)!=0 ){
         Close(handler);
        // if SystemTags == successfull, it does that itself
       }
     }else{
  ...
  }while( bFilenameUsed && next->ln_Succ ); // only continue if we must process all files

/*
  NotifyMessage *msg;
  while( msg=(NotifyMessage*)GetMsg(Main->NotifyPort) ){
    if( msg->nm_NReq->nr_UserData == 0 ) FiList->ReadSource=TRUE;
...
*/
  ...
  if( FiList->ReadSource ){
    ...
    if( strcmp(GetSrc(),FiList->Destination) == 0 ) FiList->ReadDestination = FALSE;
  }else if( FiList->DeselectMode == DESELECT_ALL ) None();
  else if( FiList->DeselectMode == DESELECT_LIST ) DeselectList( FiList );

  // if action on other dir, don't read dest:
  CheckPath( act->Dest,act->Dest ); // get correct name to compare with destination path
  if( act->Dest[0] && (stricmp(act->Dest,GetDst()) != 0) ) FiList->ReadDestination = FALSE;
