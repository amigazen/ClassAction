...
void HandleNotify(){
 ...
 while( msg=(NotifyMessage*)GetMsg(Main->NotifyPort) ){
  if( Main->PathNotify ){
   ...
   // first test, if the path node has ! yet been removed:
   if( IsPathNodeValid(pn) ){
    // if the path == shown, we can refresh the lister:
...
*/
 ...
 if( CurrentTab[0]->Path[0] == 0 ){
   Refresh(0);
 }else if( CurrentTab[1]->Path[0] == 0 ){
  Refresh(1);
...
void FormatSize( char *buffer,float size ){
 ...
 while( count<strlen(temp) ){
  buffer[n]=temp[count];
  n++; count++;
  // check if we are on a place before a . but ! on last pos
...
void ActivateBrowser( UBYTE which ){
 ...
 if( which == 0 ){
  ...
  if( which == Main->CurrentList ) return;
...
void PopupOnCurrentFiles(){
 ...
 if( selected == 0 ) return;    // nothing chosen, go away!
 ...
 if( tn->LhaActive ){
  if( (selected == 1) && (((TLBNode*)node)->Fn->Type == TYPE_FILE) ){
  ...
  // Handling multiple files would be a problem as we must copy all files to t:
   ...
   if( selected == 1 ){           // only one entry selected, just a popup
     ...
     while( node->ln_Succ ){
       ...
       if( selected ){
         strcpy( FilePointer,((TLBNode*)node)->Fn->Filename );
         if( ! caFileTypes->BelongsToFileType( fullname,ft ) ) AllTheSame=FALSE;
       }
       node = node->ln_Succ;
     }
     if( AllTheSame ) OpenActionMenu( ft );
     else OpenActionMenu( caFileTypes->Generic );
   }else if( (((TLBNode*)node)->Fn->Type == TYPE_VOLUME)
          || (((TLBNode*)node)->Fn->Type == TYPE_DEVICE) ){
     GetAttr( LISTBROWSER_Labels,lb,(ULONG*)&node );
     node = node->ln_Succ;
     while( node->ln_Succ ){
       GetListBrowserNodeAttrs( node,LBNA_Selected,&selected,TAG_END );
       if( selected ){
         if( ((TLBNode*)node)->Fn->Type == TYPE_ASSIGN ) AllTheSame=FALSE;
       }
       node = node->ln_Succ;
     }
     ...
     while( node->ln_Succ ){
       GetListBrowserNodeAttrs( node,LBNA_Selected,&selected,TAG_END );
       if( selected ){
         if( ((TLBNode*)node)->Fn->Type == TYPE_FILE )  AllTheSame=FALSE;
       }
       node = node->ln_Succ;
     }
...
//         UnLock( argptr->wa_Lock );
    ...
    case AMTYPE_APPWINDOW:{
      ...
      if( strlen(argptr->wa_Name)!=0 ){
        ...
        if( (!Main->VerticalLists && (appmsg->am_MouseX < ((Gadget*)Main->Gadgets[LST_DIR_L])->LeftEdge+
              ((Gadget*)Main->Gadgets[LST_DIR_L])->Width))
            || (Main->VerticalLists && (appmsg->am_MouseY < ((Gadget*)Main->Gadgets[LST_DIR_L])->TopEdge+
...
//>"void MakeDateString( char *dest,struct DateStamp *ds )"
void MakeDateString( char *dest,struct DateStamp *ds ){
 DateTime dt;
 char timestr[20];
 if( ds ){
...
//  Object *image;
  ...
  while( realnode->ln_Succ ){
    ...
    if( Main->ShowIcons || (stricmp(&realnode->Filename[strlen(realnode->Filename)-5],".info") != 0) ){
...
FileNode *InsertPathFile( TPathNode *pn,char *file,char *size,UBYTE type,ULONG bits,struct DateStamp *ds,ULONG bytes,BOOL link, char *comment=NULL ){
 FileNode *newnode;
...
void SetVolumeSizeStr( char *str, InfoData *id ){
 ...
 if( size>10000000000.0 ) sprintf( s,"%1.1f GB",size/(1<<30) );
 ...
 if( lock ){
   ...
   if( lha || (!Main->UseExAll) ){
     while( !end ){
      ...
      if( !end ){
       if( fib->fib_DirEntryType>0 ){
        if( fib->fib_EntryType == ST_SOFTLINK ) Link = TRUE;
        else Link = FALSE;
        ...
        if( fib->fib_EntryType == ST_LINKFILE ) Link = TRUE;
        else Link = FALSE;
     ...
     // The same with ExAll : seems to be ! faster, so why do it?
     ...
     if( eac ){
      ...
      do{
       ...
       if( eac->eac_Entries == 0 ){
        ...
        continue;                   // ("more" == *usually* zero)
       }
       ead = (ExAllData*) buffer;
       do{
        if( ead->ed_Type>0 ){
         if( ead->ed_Type == ST_SOFTLINK ) Link = TRUE;
         else Link = FALSE;
         fn=InsertPathFile( pn,ead->ed_Name,catstr,TYPE_DIR,
                         ead->ed_Prot,(struct DateStamp*)&(ead->ed_Days),ead->ed_Size,Link,ead->ed_Comment ); // insert directory
         fn->ListerImage = Main->DirImage.IntuiImage();
        }else{
         if( ead->ed_Type == ST_LINKFILE ) Link = TRUE;
         else Link = FALSE;
         FormatSize( Size,ead->ed_Size );
         fn=InsertPathFile( pn,ead->ed_Name,Size,TYPE_FILE,
                         ead->ed_Prot,(struct DateStamp*)&(ead->ed_Days),ead->ed_Size,Link,ead->ed_Comment );
         if( Main->ClassImages ){
...
void SetCurrentTabTitle( UBYTE which,char *newtitle ){
  if( CurrentTab[which]->Gadget ){
    ...
    if( *ptr == 0 ) ptr = newtitle; // it's a volume
...
TPathNode *GetPathNode( char *path, BOOL read ){
 ...
 if( pn->ln_Succ ) return( pn );
  ...
  if( !read ) return( NULL ); // ! found !!
  // else insert because ! there:
...
/* Function: ListDir                    */
/*                                      */
/* Aim:      Read && show the given    */
/*           dir                        */
/* Input:    which=0 || 1, number of    */
/*            the browser               */
...
BOOL ListDir( UBYTE which,char *path,int check,ULONG top ){
 ...
 if( path == NULL ) path = tn->Path; // get the path from the tab
 ...
 if( RealPath[0] != 0 ){
   ...
   if( (lha&&!Main->LhaPossible) || (PathProp==PATH_NOTFOUND) ){
     if( which == 0 ) SetGadgetAttrs( (Gadget *)Main->Gadgets[STR_PATH_L],Main->IntuiWin, NULL,
 ...
 // update the string gadget && store the path in the current tab:
 if( which == 0 ){
 ...
 if( stricmp(RealPath,path2) == 0 ) PathsEqual=TRUE; // paths equal, we must update both

 // get the listbrowser list, deattach  && empty it:
 ...
 // set the correct position, if wanted, && attach the list again:
 ...
 SetGadgetAttrs( (Gadget *)lb,Main->IntuiWin, NULL,
                  ...
                  TAG_DONE );
 if( PathsEqual && (check == 1) ) ListDir( 1-which,NULL,0 );
...
BOOL ChoixDirList( UBYTE which ){
  ...
  while( nd->ln_Succ ){
    ...
    if( selected ){
      ...
      if( ((TLBNode*)nd)->Fn->Type == TYPE_FILE ) Main->SelectedFiles[which]++;
...
void LhaPopup( char *file ){
 ...
 Main->ConfirmReplace = bConfirmReplace; // ... && reset the state
 SetAttrs( Main->Win,WA_BusyPointer,FALSE,TAG_END );
...
void DoubleClickDirList( UBYTE which ){
 ...
 if( node ){
  ...
  if( column < 2 ){ // perform standard action
   if( node->Fn->Type == TYPE_FILE ){

    // if it == a file in an archive, decrunch && open popup:
    if( tn->LhaActive ){
     LhaFileDoubleClick( newpath );
    // if it == ! an archive && we are ! in arch mode, we can do the standard action
    }else{
     ...
     if( cla->IsArchive ){
       ...
       if( (cla == caFileTypes->Unknown) && (XAD_IsArchive(newpath)) ){
         ListDir( which,newpath,0 );
       }else{
         // now we have only one chance: execute the first action, if there == one:
  ...
  }else if( column == 2 ){ // count size
   if( node->Fn->Type == TYPE_DIR ) GetSize( FALSE );
  }else{
  ...
  // WbInfo(); // show the wbinfo requester to get more info || change attributes
 }
 delete lhafile;
}
//<
...
void TitleClickDirList( int list ){
 ...
 if( newmode!=-1 ){
  ...
  // now we will get the nodes in reverse order, clear the list && insert the nodes in the new order
...
void Prefs(){
 ...
 SystemTags(CA_PREFSAPP,SYS_Input,Handle,SYS_Output,NULL,SYS_Asynch,TRUE,NP_StackSize,30000,TAG_DONE);
}
//<
...
void RenameFile( TFileList *list ){
 ...
 if( pa[0] == 0 ){
   device=TRUE;
   if( list->Command == IC_ADDSUFFIX ) return; // can't add a suffix to a volume/device etc.!
 }
    ...
    if( (((TLBNode*)node)->Fn->Type != TYPE_ASSIGN) &&
     ...
     if( go ){
       if( strcmp(src,dest)!=0 ){
        ...
        if( success && (!device) && (!strstr(src,".info")) ){
...
int AskReplace( char *f1, char *f2, char *reqtext ){
 ...
 vers1[0]=0; vers2[0]=0;
 if( Main->ShowVersion || Main->ReplaceVersion ){
   if( GetVersion(f1,vers1) && GetVersion(f2,vers2) ){
 ...
 if( ! Main->ReplaceVersion ){
   ...
   if( Main->ShowVersion && vers1[0] && vers2[0] ){
     sprintf( reqmsg,"%s\n\n%s    ?\n\n%s %s %s, %s\n%s %s %s, %s",
...
BOOL EnoughSpaceForCopy(){
 if( !Main->CheckFit || CurrentTab[1-Main->CurrentList]->LhaActive ) return( TRUE );
 ...
 if( lock ){
  if( Info( lock,infd ) ){
   ...
   if( available == 0 ){
     if( (stricmp("Ram Disk:",GetDst()) == 0) || (stricmp("Ram:",GetDst()) == 0) ) result = TRUE;
     else{
...
void FileTransfer( TFileList *list ){
 ...
 // filename == a pointer to the first char of the filpart;
 ...
 if( tn->LhaActive ){
   ...
   if( list->Command != FT_COPY ) return; // || if no copy from archive
 }else if( tnd->LhaActive ){
   if( (list->Command != FT_COPY) && (list->Command != FT_DELETE) &&
       (list->Command != FT_TRASHCAN) ) return;
 }
 ...
 if( list->Command == FT_DUPLICATE ) strcpy( dest,list->Source );
 else if( list->Command == FT_TRASHCAN ){
 ...
 if( (dest[0]==0) && (list->Command != FT_DELETE) && (list->Command != FT_TRASHCAN) ) return;
 AdjustPath( dest );
 destfilename = dest + strlen(dest);

 // check if it fits:
 if( ((list->Command == FT_COPY)
     || (list->Command == FT_COPYAS)
     || (list->Command == FT_MOVE)
     || (list->Command == FT_MOVEAS)) ){
   // leave copies to ftp:, FTPMount || similar dirs:
   if( (! (ptr=stristr(list->Destination,"ftp"))) || (ptr != list->Destination) ){
     if( ! EnoughSpaceForCopy() ) return;
   }
 }
 ...
 // ask for delete:
 if( (list->Command == FT_DELETE) || (list->Command == FT_TRASHCAN) ){
   if( list->Command == FT_DELETE ){
     if( list->Count == 1 ) sprintf( fuelstring,GetCatalogStr(Main->Catalog,TXT_DELOKSINGLE,TXT_DELOKSINGLE_STR),node->Name );
     else sprintf( fuelstring,GetCatalogStr(Main->Catalog,TXT_DELOK,TXT_DELOK_STR),node->Name );
   }else{
     if( list->Count == 1 ) sprintf( fuelstring,GetCatalogStr(Main->Catalog,TXT_DELOKSINGLETRASH,TXT_DELOKSINGLETRASH_STR),node->Name );
   ...
   if( Main->ConfirmDelete ){
     if( SureReq(fuelstring,GetCatalogStr(Main->Catalog,TXT_YESNO,TXT_YESNO_STR)) == 0 ){
       delete [] fuelstring;
       return;
     }
   }
   ...
   if( list->Command == FT_DUPLICATE ) list->ReadSource = TRUE;
   else{
     list->ReadDestination = TRUE;
     if( (list->Command == FT_MOVE) || (list->Command == FT_MOVEAS) ) list->ReadSource = TRUE;
   }
...
//   if( n == 0 ) return;
 ...
 while( (! Abort) && node->ln_Succ && !CheckAbort()  ){
   if( ! node->Selected ){
     node = (TFileEntry*)node->ln_Succ;
     continue;
   }
   ...
   ok=TRUE;
   if( (list->Command == FT_DUPLICATE) || (list->Command == FT_COPYAS)
   || (list->Command == FT_MOVEAS) ){
     strcpy( temptext,node->Name );
     if( list->Command == FT_DUPLICATE ) strcat( temptext,".bak" );
...
*/
   if( ok ){
     if( (list->Command == FT_COPY) && (tn->LhaActive || tnd->LhaActive) ){
       success = CopyArchiveFile( src,dest );
     }else{
       if( ((list->Command != FT_MOVE) && (list->Command != FT_MOVEAS)) || ! Rename( src,dest ) ){
     ...
     if( success ){
       node->Success  = TRUE;
       node->Selected = FALSE;
       if( (!strstr(filename,".info")) && ProcessIcons ){
         /* icon as well */
         ...
         if( success ){
           // now deselect the .info file == it == selected
       ...
       success=TRUE; // this "success" means at least the file == copied
/*
     }else{
       if( ! Abort ){
...
void Pack(){
 ...
 if( lock = Lock(Path,ACCESS_READ) ){
  ...
  if( retco != 0 ) DisplayBeep( Main->Scr );
  Close( handler );
  CurrentDir( oldlock );
  UnLock( lock );
  if( retco == 0 ){
...
void Pat(){
 ...
 ParsePatternNoCase(pat,(UBYTE *)Pattern,208);
 ...
 while( node->ln_Succ!=NULL ){
  if( (((TLBNode*)node)->Fn->Type==TYPE_FILE)
      &&(MatchPatternNoCase((UBYTE *)Pattern,((TLBNode*)node)->Fn->Filename)==1) ){
  ...
  if( GetText(title,255,GetCatalogStr(Main->Catalog,MSG_ASSIGNTITLE,MSG_ASSIGNTITLE_STR)) ){
    ...
    while( dl=NextDosEntry(dl, LDF_ASSIGNS) ){
     ...
     if( Name[0]<256 ){
      memcpy( &Filename[0], Name + 1, Name[0] );
      Filename[Name[0]]=(char)0;
      if( stricmp(Filename,title) == 0 ) ret=1;
    }}
    UnLockDosList( LDF_ASSIGNS | LDF_READ );
    if(ret)
    ...
    if(ret){
      if(lock=Lock(GetSrc(),ACCESS_READ)){
        if( ret == 1 ){ AssignLock(title,NULL); ret=3; } // remove assign, then we have a simple assign
        if( ret == 2 ) ret=AssignAdd(title,lock);       // add assign
